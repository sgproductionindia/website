...
function defaultGenres(): array
{
    return [
        ...
        [
            ...
            'name' => 'Soundcheck',
            'slug' => 'soundcheck',
            'description' => 'Soundcheck focused releases.',
            'color' => '#00e6a8',
        ],
        [
            'id' => 'marathi',
...
function webPathToFilePath(string $webPath): ?string
{
    ...
    $root = rtrim(defined('ROOT_DIR') ? ROOT_DIR : __DIR__, '/');
    $relativePath = ltrim($webPath, '/');
    $fullPath = $root . '/' . $relativePath;

    return is_file($fullPath) ? $fullPath : null;
}

function filePathToWebPath(string $filePath, bool $leadingSlash): string
{
    $root = rtrim(defined('ROOT_DIR') ? ROOT_DIR : __DIR__, '/');
    $relativePath = ltrim(str_replace($root, '', $filePath), '/');

    return ($leadingSlash ? '/' : '') . $relativePath;
}

function convertImageToWebp(string $webPath, int $quality = 82): ?string
{
    ...
    $sourcePath = webPathToFilePath($webPath);
    ...
    try {
        ...
        return filePathToWebPath($webpPath, str_starts_with($webPath, '/'));
    } finally {
...
function preferredAdminImageSrc(string $primaryPath, ?string $webpPath = null): string
{
    $primaryPath = trim($primaryPath);
    $webpPath = trim((string) $webpPath);

    if ($webpPath !== '') {
        if (preg_match('/^https?:\/\//i', $webpPath) || webPathToFilePath($webpPath) !== null) {
            return $webpPath;
        }
    }

    if ($primaryPath !== '' && !preg_match('/^https?:\/\//i', $primaryPath) && preg_match('/\.(jpe?g|png)$/i', $primaryPath)) {
        $candidate = preg_replace('/\.(jpe?g|png)$/i', '.webp', $primaryPath);

        if (is_string($candidate) && $candidate !== '' && webPathToFilePath($candidate) !== null) {
            return $candidate;
        }
    }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'login') {
    if (ADMIN_PASSWORD === '') {
        $errors[] = 'Admin password is not configured. Set ADMIN_PASSWORD in the server environment.';
...
if ($isAuthed && $_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'upload') {
    try {
        ...
        $coverWebp = convertImageToWebp($cover);
        $previewAudio = uploadFile('audio', ['mp3'], ['audio/mpeg', 'audio/mp3', 'audio/x-mpeg'], MAX_AUDIO_BYTES, AUDIO_DIR, $trackId);
        ...
        if (!preg_match('/^\d+:\d{1,2}$/', $duration)) {
            throw new RuntimeException('Could not detect the song duration. Please try a different MP3 file.');
        }
...
if ($isAuthed && $_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'save_ad') {
    try {
        ...
        if ($uploadedAd !== null) {
            ...
            $isVideoAd = in_array($extension, ['mp4', 'webm', 'mov', 'm4v'], true);
            $advertising['mediaUrl'] = $isVideoAd ? $uploadedAd : (convertImageToWebp($uploadedAd) ?? $uploadedAd);
            $advertising['mediaType'] = $isVideoAd ? 'video' : 'image';
        }
        ...
        if ($uploadedGridAdImage !== null) {
            $gridAd['imageUrl'] = convertImageToWebp($uploadedGridAdImage) ?? $uploadedGridAdImage;
        }
...
if ($isAuthed && $_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'update_track') {
    try {
        ...
        $previewAudio = uploadOptionalFile('audio', ['mp3'], ['audio/mpeg', 'audio/mp3', 'audio/x-mpeg'], MAX_AUDIO_BYTES, AUDIO_DIR, $trackId);
        ...
        if ($cover !== null) {
            ...
            $track['cover'] = $cover;
            $track['coverWebp'] = convertImageToWebp($cover);
        }
...
if ($isAuthed && $_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'delete_track') {
    try {
        ...
        if (isset($_POST['deleteFiles'])) {
            safeDeleteUpload((string) ($track['cover'] ?? ''));
            safeDeleteUpload((string) ($track['coverWebp'] ?? ''));
            safeDeleteUpload((string) ($track['previewUrl'] ?? ''));
        }
...
if ($isAuthed && $_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'save_artist') {
    try {
        ...
        if ($artistImage !== null) {
            ...
            $artist['imageWebp'] = convertImageToWebp($artistImage);
        }
...
if ($isAuthed && $_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'delete_artist') {
    try {
        ...
        safeDeleteUpload((string) ($artists[$index]['imageWebp'] ?? ''));
...
$downloadCountFor = static function (array $track): int {
    ...
    $downloadEvents = is_array($track['downloadEvents'] ?? null) ? count($track['downloadEvents']) : 0;
...
foreach ($tracks as $trackForCount) {
    if (!is_array($trackForCount)) {
        continue;
    }

    $totalLikes += $likeCountFor($trackForCount);

    if (isset($trackForCount['downloads']) || isset($trackForCount['downloadCount']) || is_array($trackForCount['downloadEvents'] ?? null)) {
        $hasDownloadData = true;
    }

    $totalDownloads += $downloadCountFor($trackForCount);
}
...
$adMediaPreviewUrl = preferredAdminImageSrc($adMediaUrl);
...
$gridAdImagePreviewUrl = preferredAdminImageSrc($gridAdImageUrl);
...
$todayIso = date('Y-m-d');
$weekAgoIso = date('Y-m-d', strtotime('-6 days'));
$downloadChartCounts = [];
$downloadChartLabels = [];
...
$downloadChartMaxValue = max($downloadChartRawValues ?: [0]);
$downloadChartSuggestedMin = 0;
$downloadChartSuggestedMax = max(10, $downloadChartMaxValue);
$downloadChartScaleRange = max(1, $downloadChartSuggestedMax - $downloadChartSuggestedMin);
$downloadChartX = [80, 215, 350, 485, 620, 755, 875];
$downloadChartPoints = [];
$downloadChartDefaultPoints = implode(' ', array_map(static fn (int $x): string => $x . ',238', $downloadChartX));

foreach ($downloadChartRawValues as $index => $value) {
    $chartValue = max($downloadChartSuggestedMin, min($value, $downloadChartSuggestedMax));
    $y = 238 - (int) round((($chartValue - $downloadChartSuggestedMin) / $downloadChartScaleRange) * 184);
    $downloadChartPoints[] = $downloadChartX[$index] . ',' . $y;
}

$downloadChartData = [
    'points' => $hasDownloadChartData ? implode(' ', $downloadChartPoints) : $downloadChartDefaultPoints,
    'values' => $hasDownloadChartData ? array_map(static fn (int $value): string => number_format($value), $downloadChartRawValues) : array_fill(0, 7, 'N/A'),
    ...
    'suggestedMin' => $downloadChartSuggestedMin,
    'suggestedMax' => $downloadChartSuggestedMax,
    'rawValues' => $downloadChartRawValues,
    'dates' => array_keys($downloadChartCounts),
];
?>
<!doctype html>
<html lang="en">
  ...
  *, *::before, *::after { margin:0; padding:0; box-sizing:border-box; }
  ...
  .stat-card.red::before   { background: var(--sys-red); }
  .stat-card.likes::before { background: #30d158; }
  ...
  .song-row {
    position: relative;
    display: grid;
    grid-template-columns: minmax(220px,1.4fr) 132px 104px 110px 104px 104px 112px;
    gap: var(--sp-3);
    align-items: center;
    background: var(--bg-secondary);
    border: 1px solid var(--separator);
  ...
  .metric-badge.purple { color: var(--sys-purple); border-color: rgba(191,90,242,0.25); }
  ...
  @media (max-width:1180px) {
    ...
    .song-row { grid-template-columns:minmax(160px,1fr) 120px 90px 90px; }
    .song-row > div:nth-child(6), .song-row > div:nth-child(7) { grid-column:auto; }
    ...
    <?php else: ?>
      ...
      <div class="main">
        <div class="topbar">
          ...
          <div class="topbar-right"><button class="btn btn-ghost" type="button" id="topExportReport">Export Report</button><button class="btn btn-primary" id="topPrimaryAction" data-action-section="upload" type="button">+ Upload Song</button></div>
        </div>
        ...
        <div class="content">
          <section class="view-section active" id="dashboard-section" data-title="Dashboard" data-subtitle="<?= e(date('M Y')) ?>">
            <div class="stats-grid">
              <div class="stat-card cyan"><div class="stat-icon"></div><div class="stat-label">Total Downloads</div><div class="stat-value cyan"><?= e(number_format($totalDownloads)) ?></div><div class="stat-change up">N/A vs last period</div></div>
          ...
          <section class="view-section" id="artists-section" data-title="Artist Management" data-subtitle="Add artist profiles for the website">
              ...
              <form class="am-form-layout" id="artistForm" method="post" enctype="multipart/form-data">
                ...
                <div class="am-image-box">
                  ...
                  <div class="am-avatar" id="amAvatarWrap">
                    <img loading="lazy" id="artistPreviewImage" data-artist-preview="new" src="assets/artist-photo-1.svg" alt="Artist profile preview">
                  </div>
            ...
            <div class="panel">
              ...
              <div class="artist-grid" id="artistGrid" style="display:<?= $artistCount > 0 ? 'grid' : 'none' ?>">
                <?php foreach ($artists as $artist): if (!is_array($artist)) continue; $artistId=(string)($artist['id']??''); $artistImageSrc = preferredAdminImageSrc((string) ($artist['image'] ?? 'assets/artist-photo-1.svg'), (string) ($artist['imageWebp'] ?? '')); ?>
                  <article class="artist-card">
                    <div class="artist-avatar"><img loading="lazy" src="<?= e($artistImageSrc) ?>" alt="<?= e((string) ($artist['name'] ?? 'Artist')) ?>"></div>
                ...
                <div class="banner-admin-list">
                  <?php foreach (array_merge($bannerSlides, [[
                    ...
                    <?php
                      ...
                      $slidePreviewImage = preferredAdminImageSrc($slideImage);
                    ...
                    <article class="banner-admin-card" id="<?= e($cardId) ?>">
                      ...
                      <div class="banner-admin-preview" style="--banner-admin-bg:<?= e($slideBg) ?>">
                        <?php if ($slideImage !== 'assets/sg-logo.svg'): ?>
                          <img loading="lazy" decoding="async" width="132" height="74" data-banner-preview-src="/<?= e($slidePreviewImage) ?>" alt="">
                        <?php else: ?>
...
(() => {
  ...
  function analyticsPeriodLabel() {
    return document.querySelector('#analytics-section .date-chip.active')?.textContent.trim() || '7D';
  }
  ...
  function exportCSV() {
    const table = document.querySelector('#analyticsTable');
    if (!table) {
      showToast('Analytics table is not available');
      return;
    }

    const rows = [];
    const headers = Array.from(table.querySelectorAll('thead th'))
      .map(th => visibleText(th))
      .filter(text => text !== '#' && text !== 'Cover');
    ...
    table.querySelectorAll('tbody tr').forEach(tr => {
      ...
      tr.querySelectorAll('td').forEach((td, index) => {
        if (index < 2) return;
        cells.push(csvCell(visibleText(td).replace(/,/g, '')));
      });
      if (cells.length > 0) rows.push(cells.join(','));
    });

    const totals = {};
    document.querySelectorAll('#analytics-section .stat-card').forEach(card => {
      const label = visibleText(card.querySelector('.stat-label'));
      const value = visibleText(card.querySelector('.stat-value'));
      if (label) totals[label] = value;
    });

    rows.push('');
    rows.push([csvCell('Totals'), csvCell('')].join(','));
    Object.keys(totals).forEach(label => {
      rows.push([csvCell(label), csvCell(totals[label])].join(','));
    });
    rows.push('');
    rows.push([csvCell('Report generated'), csvCell(new Date().toLocaleDateString())].join(','));
    rows.push([csvCell('Period'), csvCell(analyticsPeriodLabel())].join(','));

    const blob = new Blob([rows.join('\n')], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = 'sg-production-analytics-' + new Date().toISOString().slice(0, 10) + '.csv';
  ...
  function exportPDF() {
    if (!window.jspdf || !window.jspdf.jsPDF) {
      showToast('PDF exporter is still loading. Try again in a moment.');
      return;
    }
    ...
    doc.text('SG Production', margin, y);
    y += 8;

    doc.setFontSize(11);
    doc.setFont('helvetica', 'normal');
    doc.setTextColor(102, 102, 102);
    doc.text('Analytics Report - ' + analyticsPeriodLabel() + ' period - ' + new Date().toLocaleDateString(), margin, y);
    y += 6;
    ...
    const kpis = Array.from(document.querySelectorAll('#analytics-section .stat-card')).map(card => ({
      label: visibleText(card.querySelector('.stat-label')),
      value: visibleText(card.querySelector('.stat-value')) || 'N/A',
      change: visibleText(card.querySelector('.stat-change'))
    })).slice(0, 6);

    const kpiWidth = (pageWidth - margin * 2 - 10) / Math.max(1, Math.min(kpis.length, 6));
    kpis.forEach((kpi, index) => {
      const x = margin + index * (kpiWidth + 2);
      doc.setFillColor(248, 249, 250);
      doc.setDrawColor(224, 224, 224);
      doc.roundedRect(x, y, kpiWidth - 2, 22, 2, 2, 'FD');
      doc.setFontSize(16);
      doc.setFont('helvetica', 'bold');
      doc.setTextColor(17, 17, 17);
      doc.text(kpi.value, x + (kpiWidth - 2) / 2, y + 10, { align: 'center' });
      doc.setFontSize(7);
      doc.setFont('helvetica', 'normal');
      doc.setTextColor(136, 136, 136);
      doc.text(kpi.label, x + (kpiWidth - 2) / 2, y + 16, { align: 'center', maxWidth: kpiWidth - 6 });
      doc.setFontSize(6.5);
      const isUp = kpi.change.indexOf('up') >= 0 || kpi.change.indexOf('↑') >= 0 || kpi.change.toLowerCase().indexOf('live') >= 0;
      doc.setTextColor(isUp ? 46 : 255, isUp ? 189 : 69, isUp ? 107 : 58);
      doc.text(kpi.change || 'N/A', x + (kpiWidth - 2) / 2, y + 20, { align: 'center', maxWidth: kpiWidth - 6 });
    });
    y += 30;
    ...
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(17, 17, 17);
    doc.text('Top Songs by Downloads', margin, y);
    y += 6;
    ...
    if (table) {
      table.querySelectorAll('thead th').forEach(th => {
        const text = visibleText(th);
        if (text !== 'Cover') head.push(text);
      });

      table.querySelectorAll('tbody tr').forEach(tr => {
        const row = [];
        tr.querySelectorAll('td').forEach(td => {
          if (td.querySelector('.table-cover')) return;
          row.push(visibleText(td));
        });
    ...
    doc.text('Generated by SG Production Admin Panel - ' + new Date().toLocaleString(), margin, finalY + 5);

    doc.save('sg-production-analytics-' + new Date().toISOString().slice(0, 10) + '.pdf');
  ...
  const convertCoversButton = document.querySelector('#convertCoversButton');
  convertCoversButton?.addEventListener('click', () => {
    const originalText = convertCoversButton.textContent;
    convertCoversButton.disabled = true;
    convertCoversButton.textContent = 'Converting...';

    fetch('api/convert-covers.php', { method: 'POST' })
      ...
      .then((data) => {
        ...
        showToast('Converted ' + Number(data.converted || 0).toLocaleString() + ' images to WebP');
      })
      .catch(() => showToast('WebP conversion could not be completed.'))
      .finally(() => {
        convertCoversButton.disabled = false;
        convertCoversButton.textContent = originalText;
      });
  });

  // Fetch website visit stats
  fetch('api/get-visits.php')
    .then(function(response) { return response.json(); })
    .then(function(data) {
      const visitCard = document.querySelector('#visitCount');
      if (visitCard) visitCard.textContent = Number(data.allTime || 0).toLocaleString();

      const visitChange = document.querySelector('#visitChange');
      if (visitChange) {
        const change = Number(data.change7 || 0);
        const arrow = change >= 0 ? '↑' : '↓';
        visitChange.textContent = arrow + ' ' + Math.abs(change) + '% vs last week';
  ...
  fetchAdminLikes()
    .then(function(payload) {
      ...
      tracks.forEach(function(track) {
        ...
        if (!id) return;
        var badge = document.getElementById('song-likes-' + id);
        if (badge) badge.textContent = count.toLocaleString() + ' likes';
  ...
  uploadPreviewFile?.addEventListener('change', () => {
    ...
    if (!file.name.toLowerCase().endsWith('.mp3')) {
      ...
      if (previewFileName) previewFileName.textContent = 'Please choose an MP3 file';
      if (previewFileSize) previewFileSize.textContent = 'WAV is not accepted';
      showToast('Preview song must be an MP3 file');
      return;
    }
    previewFileMeta?.classList.add('is-visible');
  ...
  function getFilteredSongRows() {
    ...
    rows.sort((a, b) => {
      ...
      if (sort === 'likes') return Number(b.dataset.likes || 0) - Number(a.dataset.likes || 0);
  ...
  document.querySelectorAll('#adPerformanceExportCsv, #advertiserExportCsv, #analyticsExportCsv, #dashboardExportCsv').forEach(button => {
    button.addEventListener('click', exportCSV);
  });
  document.querySelectorAll('#advertiserDownloadPdf, #topExportReport').forEach(button => {
